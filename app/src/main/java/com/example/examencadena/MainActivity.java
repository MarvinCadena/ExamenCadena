package com.example.examencadena;
...
import com.example.examencadena.ml.Cadena;
...
public class MainActivity extends AppCompatActivity implements OnSuccessListener<Text>, OnFailureListener {
    ...
    public void onSuccess(Text text) {
        ...
        String resultados="";
        if (blocks.size() == 0) {
            resultados = "No se encontro texto";
        }else{
    ...
    public void PersonalizedModel(View v){
        try {
            ...
            String[] etiquetas = {"POLIDEPORTIVO","PARQUEADERO","SUELOS DE LABORATORIO","LABORATORIO INDUSTRIAL","LABORATORIO ACUICULTURA","LABORATORIO AGROINDUSTRIAL\n" +
                    "FCI"," FCIP", " COMEDOR"," CENTRO MEDICO" ," BAÑO" , " AUDITORIO"};

                Cadena model = Cadena.newInstance(getApplicationContext());
            TensorBuffer inputFeature0 = TensorBuffer.createFixedSize(new int[]{1, 224, 224, 3}, DataType.FLOAT32);
            inputFeature0.loadBuffer(convertirImagenATensorBuffer(mSelectedImage));

            Cadena.Outputs outputs = model.process(inputFeature0);
